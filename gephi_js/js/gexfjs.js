...
function traceMap() {
    ...
    for (var i in GexfJS.graph.edgeList) {
        ...
        if ( ( _isLinked || _displayEdges ) && ( _ds.withinFrame || _dt.withinFrame ) &&  _ds.visible && _dt.visible ) {
            ...
            GexfJS.ctxGraphe.strokeStyle = ( _isLinked ? _d.color : "rgba(100,100,100,0.4)" );
            traceArc(GexfJS.ctxGraphe, _coords, _coordt);
        }
    }
    GexfJS.ctxGraphe.lineWidth = 4;
    GexfJS.ctxGraphe.strokeStyle = "rgba(0,100,100,0.2)";
        ...
        if (_d.visible && _d.withinFrame) {
            if (i != _centralNode) {
                ...
                if (_fs > _limTxt) {
                    ...
                    GexfJS.ctxGraphe.font = Math.floor( _fs )+"px Oswald";
    ...
    if (_centralNode != -1) {
        ...
        GexfJS.ctxGraphe.font = "300 " + Math.floor( _fs )+"px Oswald";
        GexfJS.ctxGraphe.textAlign = "center";
        GexfJS.ctxGraphe.textBaseline = "middle";
        GexfJS.ctxGraphe.fillStyle = "rgba(255,255,250,0.0)";  //above is the hover text stroke
